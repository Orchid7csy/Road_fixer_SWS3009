...
for i, image_path in enumerate(image_files):
    ...
    try:
        ...
        for j, r in enumerate(results):
            ...
            # --- 后处理：在YOLO结果图上做直线检测并绘制 ---
            im_bgr_lines = im_bgr.copy()
            gray = cv2.cvtColor(im_bgr_lines, cv2.COLOR_BGR2GRAY)
            edges = cv2.Canny(gray, 50, 150)
            lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=80, minLineLength=100, maxLineGap=10)
            if lines is not None:
                for line in lines:
                    x1, y1, x2, y2 = line[0]
                    cv2.line(im_bgr_lines, (x1, y1), (x2, y2), (0, 0, 255), 2)  # 红色直线
            # --- 后处理结束 ---

            # 生成输出文件名
            base_name = os.path.splitext(os.path.basename(image_path))[0]
            output_filename = f"{base_name}_postprocessed_result_{j}.jpg"
            output_path = os.path.join(output_folder, output_filename)
            
            # 保存结果到指定文件夹
            cv2.imwrite(output_path, im_bgr_lines)
